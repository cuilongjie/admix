...
//                loadingShow: 1,      // index.html的菊花loading
                loadingStyles: {}, // 翻页时背景模糊。 add by clj. on 2018.9.13
                trans: 'fade'
            };
        },
        computed: {
        ...
        watch: {
            loading (val) { // 路由跳转，菊花的显示隐藏。 mod by clj. on 2018.9.13
            ...
            '$route.fullPath' (to, from) {
                // 根据router.beforeEach中计算的direction判断路由跳转状态，以此显示动画. mod by clj. on 2018.7.31
...
//                if (!this.$route.query.timestamp) { // 本地路由为了保存滚动位置都加了时间戳，如果没有，说明是首页~
            ...
            afterEnter (el) { // 还原滚动条信息
                ...
                setTimeout(() => { // ios低版本会出现动画延后无法正常清除webkitTransform现象，造成form页picker的遮罩层顺序颠倒，这里延迟处理。 mod by clj. on 2018.8.2
                    el.style.transform = ''; // 清除动画残留，避免合并层过多。。。
                    el.style.webkitTransform = ''; // 清除动画残留，避免合并层过多。。。
                    // el.style = null; // 清除动画残留，避免合并层过多。。。mod by clj. on 2018.7.31
                }, 500);
            },
            leave (el, done) { // 离开前获取滚动条信息
                ...
                // done();
                // 页面离开动画. mod by clj. on 2018.7.31
                let flt = 0,
                ...
                p.appendChild(cel); // 不能直接用el进行动画，这里如果异步执行done会扰乱vue的释放规则，造成滚动容器的位置会被还原，这里用了cloneNode再重新渲染一个dom进行动画展示，完事儿再异步删除。 Author by clj. on 2018.8.1
...
<style lang="scss">
    ...
    /* 滑入滑出动画。 Author by clj. on 2017.3.28 */

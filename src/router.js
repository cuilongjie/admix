...
const router = new VueRouter({
    ...
    // base: '/', // 根目录路径，配合./config/index.js中的设置，目的在build后，能顺利部署到服务端，与后台文件不冲突。。。Author by clj. on 2017.3.28
    routes // 挂载路由集合
});

router.beforeEach((to, from, next) => {
    console.info('[router.js] router.beforeEach: ------ BEFORE ------ ', ' from: ', from, ', to: ', to, ', direct: ', router.direct(to, from), router.direct());
    if (store.state && !to.matched.some(v => v.meta.disableLoading) && router.direct(to, from) >= 0) { // 路由跳转，显示菊花~ Author by clj. on 2017.2.17, 回退时不显示菊花。。。mod by clj. on 2017.8.25
        store.commit('OPEN_LOADING');
    }

    /**
     * 离开前记录当前页面滚动条位置
     *              -- Author by clj. on 2017.3.25
    ...
    try {
        if (store.state) { // 获取"有效"滚动条容器的滚动位置
            ...
            router.direction = router.direct(to, from); // 路由跳转时记录目标页面的打开方式：进入(1)、回退(-1)还是刷新(0). mod by clj. on 2018.7.31
...
router.afterEach((to, from) => {
    ...
    /**
     * 埋点
     *              -- Author by clj. on 2017.6.1
     */

    try { // 路由跳转，隐藏菊花~ Author by clj. on 2017.2.17

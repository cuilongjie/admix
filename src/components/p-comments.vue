...
<!--//            },-->
            <!--'comment.LikeFlag' (val) { // 监听点赞，如果取消或者添加，处理对应的点赞列表。 Author by clj. on 2017.2.14-->
...
<!--//                    subjectId: this.$route.params.id, // this.subjectId,-->
                ...
                <!--}).then((res) => {-->
                    ...
                    <!--if (res && len > 0) {-->
                        <!--if (this.praiseOffset === 0) this.praiseList = []; // 因为watch了点赞状态，手动添加了假数据，这里根据列表长度进行覆盖，避免重复数据。 Author by clj. on 2017.2.15-->

    ...
    methods: {
        ...
        <pre v-highlightjs @touchend.stop><code class="html">
            if (typeof this.$parent.$parent.init === 'function') {
                this.$parent.$parent.init(); // 由于父子组件嵌套，渲染顺序原因，在此需要重新调用父组件init方法重新初始化宽度。 Author by clj. on 2017.3.10
            }
        </code></pre>
...
export default {
    ...
    methods: {
        onTransitionEnd (e) { // transitionEnd事件测试，1.需要兼容性写法触发，2.只能直接用到dom上，transition、router-view元素不行。。。Author by clj. on 2017.5.21

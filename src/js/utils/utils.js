...
export function format (url, { width = 0, height = 0, type = '', thumb = false } = {}) {
    ...
    // 避免https下访问http图片会报警告（后台存储的sku图片等）。 mod by clj. on 2018.8.16
    ...
    if (/upaiyun.com/.test(url)) { // 又拍云
        ...
        if (webp) { // 如果浏览器支持webp，使用webp格式
            ...
            if (!type && newType === 'png') type = newType; // 默认png不指定类型转换是看不到渐进效果的，这里这么写是为了兼容。 Mod by clj. on 2018.8.10

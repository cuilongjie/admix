...
Vue.use(VueLazyload, {
    ...
    filter: { // 动态设定loading状态的缩略图. mod by clj. on 2018.8.15
    ...
    adapter: {
        loaded (listener, options) { // load完成后根据加载时间判断是否从缓存加载，'cached'的class决定动画时间长短。 add by clj. on 2019.1.4
...
if (window._hmt) window._hmt.push(['_setAutoPageview', false]); // 关闭百度统计自动pv记录，在router.afterEach中去掉参数（主要是timestamp）手动传。add by clj. on 2018.12.7
